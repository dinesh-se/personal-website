...
test.describe('Header ARIA Labels', () => {
	...
	test('dark mode toggle has aria-label "Toggle dark mode"', async ({ page }) => {
	...
	test('mobile menu toggle has aria-label "Open main menu" when closed', async ({ page }) => {
	...
	test('mobile menu toggle has aria-label "Close main menu" when open', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 812 });
		await page.reload();

		await page.getByRole('button', { name: 'Open main menu' }).click();

		const closeMenuButton = page.getByRole('button', { name: 'Close main menu' });
		await expect(closeMenuButton).toBeVisible();
		await expect(closeMenuButton).toHaveAttribute('aria-label', 'Close main menu');
	});

	test('mobile menu toggle aria-label toggles between open and close states', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 812 });
		await page.reload();
	...
	test('mobile menu container has aria-label "Main navigation"', async ({ page }) => {
		await page.setViewportSize({ width: 375, height: 812 });
		await page.reload();

		const menuContainer = page.locator('#mobile-menu');
		await expect(menuContainer).toHaveAttribute('aria-label', 'Main navigation');
	});
	...
	test('all header buttons are discoverable by role and name', async ({ page }) => {
		// Dark mode toggle
